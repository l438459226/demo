...
void SSD2828GPIOConfig(void)	//PA3-SPI_CS    PA5-SPI_CLK   PA6-MISO   PA7-MOSI
{
	...
	GPIO_InitTypeDef  GPIO_InitStructure;
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_AFIO, ENABLE); //使能PORTA,和AFIO时钟
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_15|GPIO_Pin_0;	   ///PORTC 3 5 6 7 复用推挽输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure); //GPIOA	
	
	
// 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14;	   ///PORTC 3 5 6 7 复用推挽输出
// 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   
// 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
// 	GPIO_Init(GPIOB, &GPIO_InitStructure); //GPIOA	
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_14;//PA6
...
void W_byte(unsigned char cmd)	//Write_SSPI_REG
{   
		u8 i;
...
void ssd_wdata(u8 reg,u16 parmer)		//
{
	...
	SSD_CS = 0;
	W_byte(0x70);	//写寄存器  W_REG sWrite_SSPI_REG
	W_byte(0x00);
	W_byte(reg);
	SSD_CS = 1;
	//delay_us(10);
...
u16 ssd_rdata(u8 reg)
{
	...
	SSD_CS = 0;//GPIO_ResetBits(GPIOA,GPIO_Pin_3);	//SSD_CS = 0;
	
	W_byte(0x70);	//写寄存器
	W_byte(0x00);
	W_byte(reg);
	
	SSD_CS = 1;//GPIO_SetBits(GPIOA,GPIO_Pin_3);	//SSD_CS = 1;
	delay_us(1);
	SSD_CS = 0;//GPIO_ResetBits(GPIOA,GPIO_Pin_3);	//SSD_CS = 0;
...
u8 DCS_Short_Read_NP(u16 cmd, u8 cnt, u8 *val)
{
	...
	do
	{
		ssd_wdata(0xb7,0x07d0);					//LP
		//ssd_wdata(0xb7,(0x03c2&0xEF)|0x03);		//HS
		//ssd_wdata(0xb7,(0x03c2&0xEF)|0x0B);		//VD
		state = ssd_rdata(0xb7);	
		printf("ssd 0xb7:0x%x",state);
		ssd_wdata(0xbb,0x000f);			//PL clock
		...
		state = ssd_rdata(0xc6);		//读 ssd2828 的状态寄存器 ssd_rdata
		if(state & 0x01)
		{
				printf("ssd read seccess\r\n\r\n");
				break;    //讀成功 跳出循環
		}
		else 
			if(++timeout_cnt > 10){
					//W_REG(0x00b7,0x03,0x0B);
					printf("ssd read failed\r\n");
...
unsigned int GP_R(uint8_t adr, uint16_t l, uint8_t *p)
{
	...
	do
	{
		...
		ssd_wdata(0xc1,l);		//Maximum Return Size
		ssd_wdata(0xc0,0x0001);		//取消SSD2828的操作？？
		ssd_wdata(0xBC,0x0001);		 // *参数个数 
		ssd_wdata(0xBF, adr);	   //把要讀的地址發送出去
				state = ssd_rdata(0xC1);	
		printf("ssd 0xC1:0x%x\r\n",state);
		delay_ms(10);					   //讀需要一点点时间
		state = ssd_rdata(0xc6);		//读 ssd2828 的状态寄存器

		if(state & 0x01)	break;     //讀成功 跳出循環
		else if(++timeout_cnt > 10){printf("ssd read failed :0x%x\r\n",state);return MIPI_READ_FAIL;}		//超時 讀失敗
