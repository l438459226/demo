	...
	do
	{
		...
		if(state & 0x01)
			...
			if(++timeout_cnt > 10){
					...
					printf("read ssd error:0x%x",state);
...
//	uint8_t debug_enabled = 0;
	uint16_t mode = 0; //0 - cmd mode, 1 - video mode
//	uint16_t fpga_r11;
	uint16_t timeout=0;
//		uint16_t state;
	u8 hdat,datl;
//	unsigned int timeout_cnt=0;
	...
	{//video mode
		...
		//printf("Init. 0xB6 == 0x%04X\r\n",rb6);
		//printf("Init. 0xB7 == 0x%04X\r\n",rb7);
		//Set REN bit, Set to READ
		rb7|=0x80;
		
		//Set 0xC1 Maximum Return Size Register
		//1. WriteSSD2828Reg(0xC1,len);
		ssd_wdata(0xC1,len);
			
		//SET Packet Size Control Register 1 (Transmit Data Count, TDC)
		//2. WriteSSD2828Reg(0xBC,0x0001);
		ssd_wdata(0xBC,0x0001);
		
		//2. Check dataType
		if(dataType==0x06){//DCS
			rb7|=0x40;
		}
		else if(dataType==0x04||dataType==0x14||dataType==0x24){//generic
			rb7&=~0x40;
		}else{
			return -1;
		}
		...
		if(hs_mode){
			rb7|=0x01;
			rb6&=~0x40;//Non video data will be transmitted using HS mode.
		}else{
			rb7&=~0x01;
			rb6|=0x40; //Non video data will be transmitted using LP mode.
		}
		
		//6. Check if Video Mode is ON
		if(rb7&0x08){//1  - Video mode is enabled
			rb7|=0x09; //Control SSD2828 use HS mode to send data.
		}
// 		printf("Set: rb6 = 0x%x, rb7 = 0x%x\r\n",rb6,rb7);
		//7. Write rb6, rb7
		ssd_wdata(0xb6,rb6);
		ssd_wdata(0xb7,rb7);
	...
	//4. WriteSSD2828Reg(0xBF,reg); Generic Packet Drop Register
	ssd_wdata(0xBF,addr);
	...
	//5.var ret=ReadSSD2828Reg(0xFF);
	//WriteSSD2828Cmd(reg);
	ssd_wuparme(0x70,0x00FF);	//ssd_wuparme(0x70,0x00FF);	
	for(i = 0; i < len;i++)
	{
		SSD_CS = 0;
		W_byte(0x73);	//读数据指令
